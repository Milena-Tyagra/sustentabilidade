export const messages = {
  TITLE: "Fibra de bananeira",
  SUBTITLE: "Inovação sustentável a partir do campo",

  // home
  PROBLEM: `Todos os anos, milhões de toneladas de pseudocaule da bananeira são descartadas no Brasil após a colheita dos frutos.
                Esse material, rico em fibras naturais, representa um enorme desperdício e gera impacto ambiental, mesmo sendo uma biomassa com alto potencial ecológico e econômico.`,
  SOLUTION: `A partir de pesquisa e práticas sustentáveis, estudantes do 9º ano da Escola Eliseu Guilherme (Ibirama/SC) desenvolveram formas de reaproveitar a fibra da bananeira,
                transformando-a em produtos ecológicos e comercializáveis.`,
  PRODUCTS_EXAMPLES: "Entre os produtos criados estão:",
  PRODUCT_1: "Papel artesanal resistente e de textura diferenciada;",
  PRODUCT_2: "Tijolos ecológicos com fibras integradas;",
  PRODUCT_3: "Esponjas biodegradáveis;",
  PRODUCT_4: "Cordas e fios trançados;",
  PRODUCT_5: "Peças decorativas e utilitárias;",
  PRODUCT_6: "Conserva do palmito da bananeira;",
  PRODUCT_7: "Xarope natural produzido com o coração da bananeira.",
  IMPACTS: "Principais impactos",

  BENEFIT_1:
    "Redução do descarte de resíduos agroindustriais e promoção de práticas mais conscientes.",

  BENEFIT_2:
    "Valorização da cultura local e dos conhecimentos populares, fortalecendo a identidade comunitária.",

  BENEFIT_3:
    "Criação de alternativas de renda por meio de produtos sustentáveis, acessíveis e de baixo custo.",

  CALL_TO_ACTION_HOME_MESSAGE: `A fibra da bananeira representa muito mais do que reaproveitamento: ela é ciência, inovação e transformação social.
    Apoie essa iniciativa e ajude a espalhar a sustentabilidade pelo campo.
    ` ,

  // art
  ART_OPENING_TEXT: ` O reaproveitamento do pseudocaule da bananeira permitiu transformar um resíduo agrícola em peças criativas, funcionais e ecológicas. Cada produto carrega ciência,
        tradição e inovação, valorizando os saberes populares e incentivando o protagonismo estudantil.`,
  CREATED_PRODUCTS: 'Produtos Criados',
  ART_TITLE_1: 'Papel artesanal',
  ART_PRODUCT_1: 'Produzido a partir da trituração da fibra, com resistência e textura diferenciada.',
  ART_TITLE_2: 'Tijolos ecológicos',
  ART_PRODUCT_2: 'Fibras integradas à massa, compactados manualmente e secos naturalmente.',
  ART_TITLE_3: 'Esponjas biodegradáveis',
  ART_PRODUCT_3: 'Alternativa ecológica ao plástico, feitas a partir das fibras.',
  ART_TITLE_4: 'Cordas e fios trançados',
  ART_PRODUCT_4: 'Resistentes, rústicos e sustentáveis.',
  ART_TITLE_5: 'Peças decorativas e utilitárias',
  ART_PRODUCT_5: 'Tags, embalagens, acessórios e objetos criativos.',
  ART_TITLE_6: 'Conserva do palmito da bananeira',
  ART_PRODUCT_6: 'Aproveitamento alimentar sustentável.',
  ART_TITLE_7: 'Xarope natural',
  ART_PRODUCT_7: 'Preparado com o coração da bananeira, destacando o uso integral da planta.',

  ART_PROCESS: 'O Processo Artesanal',
  ART_STEP_1: 'Extração do pseudocaule nas propriedades locais;',
  ART_STEP_2: 'Separação manual das fibras;',
  ART_STEP_3: 'Secagem natural ao sol e à sombra;',
  ART_STEP_4: 'Tratamento com água e imersão;',
  ART_STEP_5: 'Transformação em produtos artesanais',

  CALL_TO_ACTION_ART_MESSAGE: `A fibra da bananeira representa muito mais do que reaproveitamento: ela é ciência, inovação e transformação social.
    Apoie essa iniciativa e ajude a espalhar a sustentabilidade pelo campo.`,

  MATERIALS: "Materiais necessários:",
  ART_PRODUCT_2_MATERIAL_1: "Fibra de bananeira seca e picada (bem fina ou moída)",
  ART_PRODUCT_2_MATERIAL_2: "Solo argiloso (terra com boa liga)",
  ART_PRODUCT_2_MATERIAL_3: "Areia (fina)",
  ART_PRODUCT_2_MATERIAL_4: "Água",
  ART_PRODUCT_2_MATERIAL_5: "Forma de tijolo (pode ser de madeira ou tipo forma de tijolo ecológico)",
  ART_PRODUCT_2_MATERIAL_6: "Pá e balde",
  ART_PRODUCT_2_MATERIAL_7: "Luvas (para proteger as mãos)",

  STEPS: 'Passo a passo',

  ART_PRODUCT_2_STEP_1_TITLE: "Preparar a fibra",
  ART_PRODUCT_2_STEP_1_1: "Seque bem a fibra da bananeira ao sol",
  ART_PRODUCT_2_STEP_1_2: "Triture ou pique bem fino, como se fosse palha. Quanto mais seca e fina, melhor a mistura.",
  ART_PRODUCT_2_STEP_1_3: "Quanto mais seca e fina, melhor a mistura.",
 
  ART_PRODUCT_2_STEP_2_TITLE: "Misturar os ingredientes secos",
  ART_PRODUCT_2_STEP_2_1: "Em uma bacia ou no chão limpo, misture a terra, a areia e a fibra picada até ficar homogêneo.",
  ART_PRODUCT_2_STEP_3_TITLE: "Adicionar a água",
  ART_PRODUCT_2_STEP_3_1: "Vá adicionando água aos poucos e mexa até a mistura ficar com consistência de farofa úmida.",
  ART_PRODUCT_2_STEP_3_2: "A massa deve modelar bem sem esfarelar.",
  ART_PRODUCT_2_STEP_4_TITLE: "Moldar os tijolos",
  ART_PRODUCT_2_STEP_4_1: "Preencha a forma com a massa, pressionando bem para evitar bolhas de ar.",
  ART_PRODUCT_2_STEP_4_2: "Alise a superfície e retire com cuidado após alguns minutos.",
  ART_PRODUCT_2_STEP_5_TITLE: "Secagem",
  ART_PRODUCT_2_STEP_5_1: "Deixe os tijolos secando à sombra por 48 a 72 horas em local ventilado e coberto.",
  ART_PRODUCT_2_STEP_5_2: "Depois, exponha ao sol por 7 a 14 dias, virando-os de vez em quando.",

  ART_PRODUCT_2_IMPORTANT_NOTE: "Não use os tijolos antes da cura completa para garantir sua resistência.",

ART_PRODUCT_1_MATERIAL_1: "Fibra de bananeira (pode ser do pseudocaule ou folhas fibrosas)",
ART_PRODUCT_1_MATERIAL_2: "Água quente",
ART_PRODUCT_1_MATERIAL_3: "Liquidificador ou pilão",
ART_PRODUCT_1_MATERIAL_4: "Uma bacia grande ou balde",
ART_PRODUCT_1_MATERIAL_5: "Tela fina ou peneira (tipo aquelas de bordado ou uma tela de mosquito limpa)",
ART_PRODUCT_1_MATERIAL_6: "Toalhas ou panos limpos",
ART_PRODUCT_1_MATERIAL_7: "Rolo de madeira ou garrafa para prensar",
ART_PRODUCT_1_MATERIAL_8: "Espátula ou colher",
ART_PRODUCT_1_MATERIAL_9: "Papel toalha ou jornal para secagem",

  ART_PRODUCT_1_STEP_1_1: "Pegue a fibra da bananeira e corte em pedaços pequenos (cerca de 2-3 cm).",
 
  ART_PRODUCT_1_STEP_2_TITLE: "Cozimento e trituração das fibras",
  
  ART_PRODUCT_1_STEP_2_1: "Cozinhe as fibras na água e depois escorra",
  ART_PRODUCT_1_STEP_2_2: "Coloque os pedaços de fibra no liquidificador",
  ART_PRODUCT_1_STEP_2_3: "Adicione água até cobrir a fibra (cerca de 2 partes de água para 1 parte de fibra).",
  ART_PRODUCT_1_STEP_2_4: "Bata até formar uma polpa homogênea, parecida com uma pasta grossa.",
  ART_PRODUCT_1_STEP_3_TITLE: "Preparar a bacia para formar as folhas",
  ART_PRODUCT_1_STEP_3_1: "Encha a bacia ou balde com água.",
  ART_PRODUCT_1_STEP_3_2: "Despeje a polpa dentro da bacia e misture bem para dispersar a fibra na água.",
  ART_PRODUCT_1_STEP_4_TITLE: "Formar a folha de papel",
  ART_PRODUCT_1_STEP_4_1: "Mergulhe a tela fina ou peneira na bacia horizontalmente, de modo que a polpa se acumule sobre ela formando uma camada.",
  ART_PRODUCT_1_STEP_4_2: "Levante a tela lentamente, deixando o excesso de água escorrer.",
  ART_PRODUCT_1_STEP_5_TITLE: "Prensar a folha",
  ART_PRODUCT_1_STEP_5_1: "Coloque a tela com a polpa sobre um pano limpo.",
  ART_PRODUCT_1_STEP_5_2: "Cubra com outro pano e use o rolo de madeira ou garrafa para pressionar, retirando o excesso de água.",
  ART_PRODUCT_1_STEP_6_1: "Com cuidado, retire a folha de papel da tela e coloque sobre uma superfície plana coberta com papel toalha ou jornal.",
  ART_PRODUCT_1_STEP_6_2: "Deixe secar por 24 a 48 horas, dependendo da umidade e temperatura.",
  ART_PRODUCT_1_STEP_6_3: "Para acelerar, pode secar no sol.",

  ART_PRODUCT_1_IMPORTANT_NOTE: "Não use os tijolos antes da cura completa para garantir sua resistência.",

};
